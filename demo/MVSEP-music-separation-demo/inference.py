...
def run_inference(input_audio, output_folder, cpu=False, overlap_large=0.6, overlap_small=0.5, single_onnx=False, 
                  chunk_size=1000000, large_gpu=False, use_kim_model_1=False, only_vocals=False):
    start_time = time()

    print("Version: {}".format(__VERSION__))  # You can replace __VERSION__ with your version string if needed.
    ...
    options = {
        ...
        'overlap_small': overlap_small,
        'single_onnx': single_onnx,
        'chunk_size': chunk_size,
        'large_gpu': large_gpu,
        'use_kim_model_1': use_kim_model_1,
        'only_vocals': only_vocals
    }
    
    # Print the options dictionary for debugging purposes
    print("Options: ")
    for el in options:
        print('{}: {}'.format(el, options[el]))

    # Assuming predict_with_model is a function defined elsewhere
...
if __name__ == '__main__':
    ...
    run_inference(
        ...
        output_folder=options.output_folder,
        cpu=options.cpu,
        overlap_large=options.overlap_large,
        overlap_small=options.overlap_small,
        single_onnx=options.single_onnx,
        chunk_size=options.chunk_size,
        large_gpu=options.large_gpu,
        use_kim_model_1=options.use_kim_model_1,
        only_vocals=options.only_vocals
    )
